...
public:
	Board(char p, bool ai) : player(p), isPlayer(ai) {} //Konstruktor parametryczny
...
private:
	...
	int AI() {
		...
		cout << this->userChoices;

		system("pause");
		cout <<"Losowanko: "<< aiChoice;
		return aiChoice;
	}
	...
	void Input() {
		if (this->isPlayer == true) {
			...
			if (this->aiTurn == true) {
				cout << "AI turn" << endl;
				system("pause");
				this->userFieldChoice = this->AI();
			}
	
			else {
				cout << "\n\n Wybierz pole: ";
				cin >> this->userFieldChoice;
				this->userChoices.push(this->userFieldChoice);
			}
...
int main() {
	...
	if (gamechoice == 1)
	{
		Board game(WhichPlayer(), true); //Instancja klasy, wywo³uje konstruktor parametryczny, parametr jest zwracany z funkcji WhichPlayer;
		game.TicTacToeGame();
	}
	else if (gamechoice == 2)
	{
		Board game(WhichPlayer(), false); //Instancja klasy, wywo³uje konstruktor parametryczny, parametr jest zwracany z funkcji WhichPlayer;
		game.TicTacToeGame();
	}
